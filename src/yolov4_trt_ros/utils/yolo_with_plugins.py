...
>>>>>>> dda16fa5ba7bc80db822cd6a331011c004000543
...
<<<<<<< HEAD
    def preprocess_yolo(self, img, input_shape):
        """Preprocess an image before TRT YOLO inferencing.
    ...
    def postprocess_yolo(self, trt_outputs, img_w, img_h, conf_th, nms_threshold=0.5):
        """Postprocess TensorRT outputs.
    ...
    def detect(self, img_resized, img_shape, conf_th=0.3):
...
<<<<<<< HEAD
        boxes, scores, classes = self.postprocess_yolo(
            trt_outputs, img_shape[1], img_shape[0], conf_th)

        # clip x1, y1, x2, y2 within original image
        boxes[:, [0, 2]] = np.clip(boxes[:, [0, 2]], 0, img_shape[1]-1)
        boxes[:, [1, 3]] = np.clip(boxes[:, [1, 3]], 0, img_shape[0]-1)
=======
